...
extension TestSeriesVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.lblNoOfTest.text = "\(obj.num ?? 0)"
    ...
    func apiAllTestSeries() {
        ...
        ApiManager.networdRequest(requestType: HttpRequestType.POST, apiUrl: ApiManager.allTestSeries, inputParam: param) { (jsonResponse, error, success) in
            if success {
                if let response = jsonResponse {
                    let seriesData = AllTestSeriesBaseModel(response: response)
